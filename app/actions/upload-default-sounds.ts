  ...
  try {
    ...
    const uploadPromises = DEFAULT_SOUNDS.map(async (sound) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to fetch sound: ${sound.url} (${response.status}: ${response.statusText})`)
        }

        const file = await response.blob()
        console.log(`Successfully fetched sound: ${sound.url}, size: ${file.size} bytes`)
    ...
    return {
      success: false,
      message: `Error uploading default sounds: ${error instanceof Error ? error.message : String(error)}`,
    }
  }
}
...
export async function getSounds(forceRefresh = false): Promise<SoundMetadata[]> {
  try {
